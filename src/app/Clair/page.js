...
const ClairImageSec = () => {
    ...
    async function checkImageSec() {
        ...
        try {
            ...
            const data = await axios.post('http://localhost:4000/secure-image-clair', imageData)
            setLoading("Scan Complete")
            console.log("Returned Data: ", data)
            try {
                const fileData = await axios.get(`http://localhost:4000/read-file`,{ params: { fileName: 'scan-report-clair.json' } });
    ...
    return (
        <div className="p-10 flex min-h-screen flex-col  bg-slate-800">
            <div className="flex flex-row justify-start items-end gap-10">
                ...
                <button onClick={checkImageSec} className="bg-white rounded-lg px-4 py-2 h-10 text-slate-800 text-xl font-bold">Scan With Clair</button>
            </div>
